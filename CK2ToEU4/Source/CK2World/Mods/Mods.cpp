...
void CK2::Mods::loadCK2ModDirectory(const Configuration& theConfiguration)
{
	...
	for (const auto& filename: filenames) {
		if (fs::path(filename).extension() != ".mod") continue;
		...
		try {
			...
			if (theMod.looksValid()) {
				if (!theMod.isCompressed()) {
					...
					Log(LogLevel::Info) << "\t\tFound potential mod named " << theMod.getName() << " with a mod file at " << CK2ModsPath
					...
					Log(LogLevel::Info) << "\t\tFound a compessed mod named " << theMod.getName() << " with a mod file at " << CK2ModsPath << "/" << filename
...
std::optional<std::string> CK2::Mods::getModPath(const std::string& modName) const
{
	...
	if (compressedMod != possibleCompressedMods.end()) {
		...
		if (pos != std::string::npos) { uncompressedName = uncompressedName.substr(pos + 1, uncompressedName.size()); }
		...
		if (Utils::doesFolderExist("mods/" + uncompressedName)) { return "mods/" + uncompressedName; }
	}
