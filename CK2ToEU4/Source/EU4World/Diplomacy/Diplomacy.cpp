...
void EU4::Diplomacy::importTributaries(const std::map<std::string, std::shared_ptr<Country>>& countries,
	 const CK2::Diplomacy& diplomacy,
	 date conversionDate)
{
	...
	for (const auto& country: countries) {
		...
		if (isCountryVassal(country.first) || isCountryJunior(country.first)) continue;
		...
		if (relationsItr == allRelations.end()) continue; // No relations.
	...
	for (const auto& holderItr: holderTributaries) {
		...
		for (const auto& subHolderItr: holderItr.second) {
			...
			if (!holderTags.count(subHolderItr.first)) continue;
			// He does! Excellent!
...
{
	for (const auto& agreement: agreements) {
		if (agreement->getSecond() == tag && agreement->getType() == "vassal") return true;
	}
	return false;
}
...
{
	for (const auto& agreement: agreements) {
		if (agreement->getSecond() == tag && agreement->getType() == "union") return true;
	}
	return false;
}
