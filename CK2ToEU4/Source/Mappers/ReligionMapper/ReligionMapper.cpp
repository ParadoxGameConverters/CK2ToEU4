...
void mappers::ReligionMapper::registerKeys()
{
	registerKeyword("link", [this](const std::string& unused, std::istream& theStream) {
		...
		for (const auto& ck2Religion: theMapping.getCK2Religions()) { ck2ToEu4ReligionMap.insert(std::make_pair(ck2Religion, theMapping.getEU4Religion())); }
	});
...
std::optional<std::string> mappers::ReligionMapper::getEu4ReligionForCk2Religion(const std::string& ck2Religion) const
{
	...
	if (mapping != ck2ToEu4ReligionMap.end()) return mapping->second;
	return std::nullopt;
}